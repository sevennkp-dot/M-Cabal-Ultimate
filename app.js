...
async function updateBuy(id, status) {
  if (status === 'completed') {
    ...
    if (order && latest) {
      ...
      // ส่งแจ้งเตือน Push Message ไปที่ LINE ลูกค้า
      if (order.line_user_id) {
        fetch(`${SUPABASE_URL}/functions/v1/line-webhook`, {
          method: 'POST',
          headers: headers,
          body: JSON.stringify({ action: 'notify_buy_complete', userId: order.line_user_id, m_amount: order.m_amount })
        }).then(res => console.log('Ping webhook:', res.status)).catch(err => console.error('Failed to send push notification:', err))
      }
    }
  }
...
async function updateSell(id, status) {
  if (status === 'completed') {
    ...
    if (order?.[0]) {
      ...
      // ส่งแจ้งเตือน Push Message ไปที่ LINE ลูกค้า
      if (order[0].line_user_id) {
        fetch(`${SUPABASE_URL}/functions/v1/line-webhook`, {
          method: 'POST',
          headers: headers,
          body: JSON.stringify({ action: 'notify_sell_complete', userId: order[0].line_user_id, total_price: order[0].total_price })
        }).then(res => console.log('Ping webhook:', res.status)).catch(err => console.error('Failed to send push notification:', err))
      }
    }
  }
